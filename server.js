...
function custoPintura(largura_cm, altura_cm, tipo) {
  ...
  var base = tipo === 'especial' ? CFG.pintEspecial : CFG.pintBasica;
...
function calcularOrcamento(itens) {
  ...
  var itensCalculados = lista.map(function(item) {
    ...
    totalMat += matUnit * qtd;
    totalPint += pintUnit * qtd;
    totalMO += moUnit * qtd;
    ...
    return {
      ...
      custo_material_unitario: matUnit,
      custo_pintura_unitario: parseFloat(pintUnit.toFixed(2)),
      mao_obra_unitaria: moUnit,
...
async function extrairDadosPDF(texto) {
  ...
  var prompt = 'Voce e especialista em orcamentos de serralheria.\n' +
    'Analise o texto abaixo e retorne APENAS um JSON valido sem explicacoes.\n\n' +
    'TEXTO:\n' + texto + '\n\n' +
    'FORMATO:\n' +
    '{"cliente":{"nome":"","telefone":"","email":""},' +
    '"itens":[{"descricao":"","largura_cm":0,"altura_cm":0,"quantidade":1,' +
    '"material":"","custo_material_unitario":0,"tipo_pintura":"basica",' +
    '"mao_obra_unitaria":0,"observacoes":null}],' +
    '"observacoes_gerais":"","confianca":"baixa"}';

  var result = await model.generateContent(prompt);
  var txt = result.response.text().trim()
    .replace(/```json\n?/g, '')
    .replace(/```\n?/g, '')
    .trim();
